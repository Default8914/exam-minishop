...
let state = {
  ...
  orders: [] // {id, createdAt, customer, total}
};
...
function renderRoute(){
  ...
  // всегда обновляем корзину в шапке/модалке
  syncCartUI();
  ...
  if(route.name === "catalog"){
    ...
    const page = viewCatalog({
      ...
      onBindFilters: ({ search, category, sort, maxPrice, reset, maxPriceLabel, found, grid }) => {
        const update = () => {
          const filtered2 = applyFilters([...products]);
          found.textContent = String(filtered2.length);
          maxPriceLabel.textContent = `до ${state.filters.maxPrice} ₽`;
          renderGrid(grid, filtered2,
  ...
  if(route.name === "product"){
    ...
    const page = viewProduct({
      product,
      onAdd: (id)=>{ addToCart(state.cart, id); persist(); syncCartUI(); openModal(els.cartModal); },
      onBack: ()=> history.length ? history.back() : (location.hash = "#/")
    });
    els.view.appendChild(page);
    return;
  }
  ...
  if(route.name === "orders"){
    const page = viewOrders({ orders: state.orders });
    els.view.appendChild(page);
    return;
  }
}
